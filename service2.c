...
int main(int argc, char *argv[])
{
  ...
  int i, fd, clfd, newfd, TCPfd_sender, TCPfd_listen, recv_bytes, sent_bytes, port, arg_count = 0, myid, start_id, id,
  service, state, myip_arg, myTCPport_arg, myUDPport_arg, counter, maxfd;
  fd_set rfds;
  socklen_t addrlen;
  struct sockaddr_in c_serveraddr, r_serveraddr, myudpaddr, mytcpaddr, clientaddr;
  struct in_addr ip;
  ...
  TCPfd_listen = socket(AF_INET,SOCK_STREAM,0);
  TCPfd_sender = socket(AF_INET,SOCK_STREAM,0);
  ...
  status=idle;
    bind(TCPfd_listen,(struct sockaddr*)&mytcpaddr, sizeof(mytcpaddr));
  ...
  while(1)
  {
    ...
    {
      FD_SET(clfd,&rfds);
      FD_SET(TCPfd_listen, &rfds);
      FD_SET(TCPfd_sender, &rfds);
      maxfd=max(clfd, TCPfd_listen);
      maxfd=max(maxfd, TCPfd_sender);
    }
...
//CHECK THIS PART ONWARDS
    ...
    listen(TCPfd_listen,5);
      ...
      newfd = accept(fd,(struct sockaddr*)&r_serveraddr,&clientlen);

      read(newfd, buffer, BUFFERSIZE);
      printf("RECEIVED MESSAGE FROM RING JOINER: %s\n", buffer);
            ...
            if (strcmp(command,"join")==0)
            {
              if(service)
              {
                ...
                if (start_id)
                {
                  ...
                  //TCP part
                  select(maxfd+1,&rfds,(fd_set*)NULL,(fd_set*)NULL,(struct timeval *)NULL);

                  if (FD_ISSET(TCPfd_sender, &rfds))
                  {
                    r_serveraddr.sin_family = AF_INET;
                    r_serveraddr.sin_addr = ip;
                    r_serveraddr.sin_port = htons((u_short)port);

                    if (connect(TCPfd_sender,(struct sockaddr*)&r_serveraddr, sizeof(r_serveraddr)) == -1)
                    {
                      perror("TROUBLE ESTABLISHING TCP CONNECTION: ");
                      break;
                    }

                    sprintf(msg,"NEW %d;%s;%d\n", service, inet_ntoa(ip), atoi(argv[myTCPport_arg]));

                    write(TCPfd_sender, msg, strlen(msg));
                  }
