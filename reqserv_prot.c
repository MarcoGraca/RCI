...
int main(int argc, char* argv[])
{
  ...
  fd_set rfds;
  FD_ZERO(&rfds);
  int fd = socket(AF_INET,SOCK_DGRAM,0), dsfd = socket(AF_INET,SOCK_DGRAM,0), recv_bytes, sent_bytes, service=0, port=PORT, dsid, dspt;
  int maxfd = 0, counter, state;
  socklen_t addrlen;
  ...
  while(1)
  {
    ...
    if (counter < 1)
    {
      printf("Couldn't contact anything\n");
      return 0;
    }
